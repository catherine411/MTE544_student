...
class particleFilter(Node):
    ...
    def initializeParticleFilter(self, x, y, th):
        ...
        self.particlePoses = np.column_stack((xs, ys, ths)) #size should be (numParticles, 3)
    ...
    def resample(self, laser_scan, mapUtilInstance):
        ...
        indices = np.random.choice(len(self.particles), size=self.numParticles, p=particles_weights)
        sampled_particles = [self.particles[i] for i in indices]

        for bp in sampled_particles:
            x, y, th = bp.getPose()
            # TODO: add noise to the x, y, and th, use the same std_noise for x, y, and th
            noise_x, noise_y, noise_th = np.random.normal(0.0, std_noise, 3)
            new_x = x + noise_x
            new_y = y + noise_y
            new_th = th + noise_th
